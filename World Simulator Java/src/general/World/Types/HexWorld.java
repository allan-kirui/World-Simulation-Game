...
import general.view.buttons.HexButton;
...
public class HexWorld extends World {
    public HexWorld(final int wid, final int len) {
        this.set_width(wid);
        this.set_length(len);
        this.set_turn(0);
        set_mode(6);
        ...
        KeyboardFocusManager.getCurrentKeyboardFocusManager()
                .addKeyEventDispatcher(e -> {
                    ...
                    if (human != null) {
                        ...
                        } else if (action == hex_up_right) {
                            wid1 = prev_wid - human.get_range();
                            len1 = prev_len + human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_up_left) {
                            wid1 = prev_wid - human.get_range();
                            len1 = prev_len - human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_down) {
                            wid1 = prev_wid + human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_down_right) {
                            wid1 = prev_wid + human.get_range();
                            len1 = prev_len + human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_down_left) {
                            wid1 = prev_wid + human.get_range();
                            len1 = prev_len - human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_right) {
                            len1 = prev_len + human.get_range();
                            get_next_round().setEnabled(true);
                        } else if (action == hex_left) {
                            len1 = prev_len - human.get_range();
                            get_next_round().setEnabled(true);
                        }
    ...
    public void draw_world() {
        ...
        if (human != null) {
            get_elixir_button().setEnabled(find_human().get_force() <= 5);
            frame.add(get_elixir_button());
        } else {
        ...
        for (int i = 0; i < get_width(); i++) {
            for (int j = 0; j < get_length(); j++) {
                ...
                //ALLOWS FOR SLANTED DESIGN
                field.setBounds(10 + (j * 50) + (i * 20), 150 + (i * 40), 40, 40);//x axis, y axis, width, height
